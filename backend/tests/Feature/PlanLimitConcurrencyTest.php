...
class PlanLimitConcurrencyTest extends TestCase
{
    ...
    public function test_fourth_simultaneous_schema_doctor_allowance_is_rejected(): void
    {
        if (! function_exists('pcntl_fork')) {
            $this->markTestSkipped('pcntl is required to verify concurrent database connections.');
        }
        ...
        try {
            ...
            for ($attempt = 0; $attempt < 4; $attempt++) {
                ...
                if ($pid === 0) {
                    DB::purge();
                    $attemptUser = User::findOrFail($user->id);
                    exit(app(PlanLimitService::class)->consumeSchemaDoctorAllowance($attemptUser) ? 0 : 1);
                }
            ...
            SchemaDoctorUsage::where('user_id', $user->id)->delete();
            $user->delete();
