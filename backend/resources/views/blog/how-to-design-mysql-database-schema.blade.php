...
@section('head')
    ...
    <meta property="og:description" content="A practical step-by-step guide covering entities, columns, data types, primary keys, foreign keys, and normalization for MySQL.">
...
<article class="blog-post">
    ...
    <p>
        For each entity, list the properties you need to store. A <strong>User</strong> might have: <code>id</code>, <code>email</code>, <code>password_hash</code>, <code>name</code>, <code>created_at</code>. A <strong>Product</strong> might have: <code>id</code>, <code>name</code>, <code>description</code>, <code>price</code>, <code>stock_quantity</code>, <code>category_id</code>.
    </p>
    <p>
        Keep column names lowercase with underscores (snake_case). Avoid abbreviations — <code>created_at</code> is better than <code>cr_at</code>.
    </p>
    ...
    <ul>
        <li><code>INT</code> or <code>BIGINT</code> — for IDs and counts. Use <code>BIGINT</code> if the table may grow very large.</li>
        <li><code>VARCHAR(n)</code> — for variable-length strings like names and emails. Set <code>n</code> to a realistic maximum.</li>
        <li><code>TEXT</code> — for long-form content like descriptions. Avoid indexing <code>TEXT</code> columns directly.</li>
        <li><code>DECIMAL(p, s)</code> — for monetary values. Never use <code>FLOAT</code> for money due to floating-point precision issues.</li>
        <li><code>TINYINT(1)</code> — for booleans (MySQL's conventional boolean type).</li>
        <li><code>DATETIME</code> or <code>TIMESTAMP</code> — for dates and times. <code>TIMESTAMP</code> stores in UTC and auto-converts on retrieval.</li>
    </ul>
    ...
    <p>
        For distributed systems or when you need to generate IDs outside the database, a <code>CHAR(36)</code> UUID column is an alternative, though it's slower to index than integers.
    </p>
    ...
    <ul>
        <li><strong>One-to-many</strong> — a User has many Orders. Add a <code>user_id</code> foreign key to the Orders table.</li>
        <li><strong>Many-to-many</strong> — an Order contains many Products, and a Product can appear in many Orders. Model this with a join table: <code>order_items</code> with <code>order_id</code> and <code>product_id</code>.</li>
        <li><strong>One-to-one</strong> — a User has one Profile. Add a <code>user_id</code> foreign key to the Profiles table with a <code>UNIQUE</code> constraint.</li>
    </ul>
    <p>
    ...
    <ul>
        ...
        <li><strong>2NF</strong> — every non-key column depends on the entire primary key (relevant for composite keys).</li>
        <li><strong>3NF</strong> — every non-key column depends only on the primary key, not on other non-key columns.</li>
    </ul>
    <p>
    ...
    <p>
        A good schema diagram shows every table with its columns and data types, with lines connecting foreign keys to their referenced primary keys. When you're happy with it, you can export a ready-to-run <code>CREATE TABLE</code> SQL script directly.
    </p>
    ...
    <div class="cta-box">
        ...
        <p>SQL Designer is a free, browser-based MySQL schema designer. Add tables, define relationships, and export a SQL script in minutes — no installation required.</p>
